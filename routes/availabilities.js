...
router.post(
  ...
  async (req, res, next) => {
    await body('availability')
      .isInt({ min: 0, max: 2 })
      .withMessage('0 以上 2 以下の数値を指定してください。')
      .run(req)
    await param('scheduleId')
      .isUUID('4')
      .withMessage('有効なスケジュール ID を指定してください。')
      .run(req)
    await param('candidateId')
      .isInt()
      .withMessage('有効な候補 ID を指定してください。')
      .run(req)
    await param('userId')
      .isInt()
      .custom((value, { req }) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ status: 'NG', errors: [{ msg: 'データベース エラー。' }] })
    }
  }
)
